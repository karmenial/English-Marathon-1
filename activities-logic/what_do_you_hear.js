...
window.activitiesLogic.what_do_you_hear = {
    // ============ تحميل الأسئلة ============
    loadQuestions: async function(level, grade, curriculum, unit) {
        ...
        try {
            ...
            if (localData) {
                ...
                if (db && db.activityDatabase && db.activityDatabase.what_do_you_hear) {
                    const filtered = db.activityDatabase.what_do_you_hear.filter(q =>
        ...
        } catch (e) { console.warn("⚠️ Error in localStorage:", e); }
        ...
        } catch (error) { console.warn("⚠️ Error in JSON:", error); }
        return [];
    },
    ...
    render: function(question, questionIndex) {
        ...
        html += `<p class="question-title-text"><b>${questionIndex + 1}.</b> Listen carefully and choose the sentence you heard:</p>`;
        
        html += `<div class="what-do-you-hear-container" id="what_do_you_hear_${qIdStr}" style="background: rgba(255,255,255,0.08); border: 3px solid var(--border-color); border-radius: 14px; padding: 20px; margin: 15px 0;">`;
        
        // ✅ الخيارات عمودية دائماً
        html += `<div class="what-do-you-hear-options" id="options_${qIdStr}" style="display: flex; flex-direction: column; gap: 12px; margin-bottom: 20px;">`;
        const shuffledOptions = this.shuffleArray([...q.options]);
        shuffledOptions.forEach(opt => {
            const cleanOpt = opt.replace(/"/g, '&quot;').replace(/'/g, '&#39;');
            html += `<button class="what-do-you-hear-option-btn" data-option="${cleanOpt}" data-qid="${qIdStr}" style="background: rgba(255,255,255,0.08); border: 2px solid var(--border-color); border-radius: 10px; padding: 14px; font-size: 1.1rem; font-weight: 700; cursor: pointer; text-align: left; transition: all 0.2s; color: var(--text-main); font-family: inherit; line-height: 1.6;">${opt}</button>`;
        });
        html += `</div>`;
        
        // ✅ الأزرار الثلاثة القياسية في صف واحد فقط
        html += `<div class="what-do-you-hear-controls" data-controls="${qIdStr}" style="display: flex; gap: 10px; flex-wrap: wrap;">`;
        html += `<button class="btn btn-info" data-action="speak" data-qid="${qIdStr}" style="flex: 1; min-width: 120px;">🔊 Listen</button>`;
        html += `<button class="btn btn-secondary" data-action="stt" data-qid="${qIdStr}" style="flex: 1; min-width: 120px;">🎤 Speak</button>`;
        html += `<button class="btn btn-primary" data-action="save" data-qid="${qIdStr}" style="flex: 1; min-width: 120px;">💾 Save</button>`;
        html += `</div>`;
    ...
    init: function(questionId) {
        ...
        if (!q.whatDoYouHearState) {
            q.whatDoYouHearState = {
                selectedOption: null,
                isSaved: false,
                recognition: null
            };
        ...
        if (optionsContainer) {
            optionsContainer.addEventListener('click', (e) => {
                const btn = e.target.closest('.what-do-you-hear-option-btn');
                if (!btn) return;

                const selectedOpt = btn.dataset.option;
                
                // إزالة التحديد السابق
                const prevSelected = optionsContainer.querySelector('.what-do-you-hear-option-btn.selected');
                if (prevSelected) {
                ...
                q.whatDoYouHearState.selectedOption = selectedOpt;
                q.whatDoYouHearState.isSaved = false;
        ...
        if (SpeechRecognition) {
            ...
            q.whatDoYouHearState.recognition = recognition;
        }
    },
    ...
    speakSentence: function(qIdStr) {
        ...
        if (window.speechSynthesis.speaking) {
            ...
            if (btn) { btn.innerHTML = '🔊 Listen'; btn.classList.remove('btn-warning'); btn.classList.add('btn-info'); }
            return;
        }
        ...
        if ('speechSynthesis' in window) {
            ...
            if (btn) { btn.innerHTML = '⏹️ Stop'; btn.classList.remove('btn-info'); btn.classList.add('btn-warning'); }
    ...
    startVoiceRecognition: function(qIdStr) {
        const q = questionsList.find(q => String(q.id) === qIdStr);
        if (!q || !q.whatDoYouHearState.recognition) {
        ...
        const recognition = q.whatDoYouHearState.recognition;
        ...
        recognition.onresult = (event) => {
            ...
            const optionButtons = document.querySelectorAll(`#options_${qIdStr} .what-do-you-hear-option-btn`);
            let matched = false;
            ...
            if (statusEl) {
                if (matched) {
                    statusEl.textContent = `✅ Selected: "${transcript}"`;
                    statusEl.style.color = '#4ade80';
                    statusEl.style.background = 'rgba(74, 222, 128, 0.1)';
                } else {
                    statusEl.textContent = `⚠️ No match found. You said: "${transcript}"`;
        ...
        recognition.onstart = () => {
            ...
            if (statusEl) {
                ...
                statusEl.textContent = '🎧 Speak the sentence clearly...';
        ...
        recognition.onerror = (event) => {
            if (statusEl) {
                statusEl.textContent = '⚠️ Could not hear you clearly. Try again or select manually.';
                statusEl.style.color = '#ef4444';
                statusEl.style.background = 'rgba(239, 68, 68, 0.1)';
            }
        };
    ...
    saveAnswer: function(qIdStr) {
        ...
        if (!q.whatDoYouHearState.selectedOption) {
            alert("Please select an answer first!");
            return;
        }

        q.whatDoYouHearState.savedOption = q.whatDoYouHearState.selectedOption;
        q.whatDoYouHearState.isSaved = true;
        ...
        if (statusEl) {
            statusEl.textContent = '💾 Your answer is saved. You can change it before finishing the exam.';
            statusEl.style.display = 'block';
    ...
    calculateScore: function(questionId) {
        const qIdStr = String(questionId);
        const q = questionsList.find(q => String(q.id) === qIdStr);
        if (!q || !q.whatDoYouHearState) return 0;

        const userAnswer = (q.whatDoYouHearState.savedOption || q.whatDoYouHearState.selectedOption || '').trim().toLowerCase().replace(/\s+/g, ' ');
        const correctAnswer = (q.sentence || '').trim().toLowerCase().replace(/\s+/g, ' ');
