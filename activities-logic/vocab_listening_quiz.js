...
window.activitiesLogic.vocab_listening_quiz = {
    // ============ تحميل الأسئلة ============
    loadQuestions: async function(level, grade, curriculum, unit) {
        ...
        try {
            ...
            if (localData) {
                ...
                if (db && db.activityDatabase && db.activityDatabase.vocab_listening_quiz) {
                    const filtered = db.activityDatabase.vocab_listening_quiz.filter(q =>
    ...
    render: function(question, questionIndex) {
        ...
        html += `<p class="question-title-text"><b>${questionIndex + 1}.</b> Listen and choose the correct meaning:</p>`;
        
        html += `<div class="vocab-listening-container" id="vocab_listening_${qIdStr}" style="background: rgba(255,255,255,0.08); border: 3px solid var(--border-color); border-radius: 14px; padding: 20px; margin: 15px 0;">`;
        
        // ✅ إخفاء الكلمة الإنجليزية تماماً (مخفية بصرياً ولكن موجودة في DOM لمحرك النطق)
    ...
    init: function(questionId) {
        ...
        if (!q.vocabListeningState) {
            q.vocabListeningState = {
                selectedOption: null,
                isSaved: false,
                recognition: null
            };
        ...
        if (optionsContainer) {
            optionsContainer.addEventListener('click', (e) => {
                const btn = e.target.closest('.vocab-listening-option-btn');
                if (!btn) return;

                const selectedOpt = btn.dataset.option;
                
                // إزالة التحديد السابق
                const prevSelected = optionsContainer.querySelector('.vocab-listening-option-btn.selected');
                if (prevSelected) {
                ...
                q.vocabListeningState.selectedOption = selectedOpt;
                q.vocabListeningState.isSaved = false;
        ...
        // 4. تهيئة التعرف على الصوت (العربية)
        const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
        if (SpeechRecognition) {
            const recognition = new SpeechRecognition();
            recognition.continuous = false;
            recognition.interimResults = false;
            recognition.lang = 'ar-SA'; // اللغة العربية للمعاني
            q.vocabListeningState.recognition = recognition;
        }
    },
    ...
    speakWord: function(qIdStr) {
        ...
        if ('speechSynthesis' in window) {
            window.speechSynthesis.cancel();
            // نقرأ الكلمة الإنجليزية المخزنة في q.word
    ...
    startVoiceRecognition: function(qIdStr) {
        const q = questionsList.find(q => String(q.id) === qIdStr);
        if (!q || !q.vocabListeningState.recognition) {
        ...
        const recognition = q.vocabListeningState.recognition;
        ...
        recognition.onresult = (event) => {
            ...
            const optionButtons = document.querySelectorAll(`#options_${qIdStr} .vocab-listening-option-btn`);
            let matched = false;
        ...
        recognition.onstart = () => {
            ...
            if (statusEl) {
                ...
                statusEl.textContent = '🎧 قل المعنى بالعربية بوضوح...';
    ...
    saveAnswer: function(qIdStr) {
        ...
        if (!q.vocabListeningState.selectedOption) {
            alert("الرجاء اختيار إجابة أولاً!");
            return;
        }

        q.vocabListeningState.savedOption = q.vocabListeningState.selectedOption;
        q.vocabListeningState.isSaved = true;
    ...
    calculateScore: function(questionId) {
        const qIdStr = String(questionId);
        const q = questionsList.find(q => String(q.id) === qIdStr);
        if (!q || !q.vocabListeningState) return 0;

        const userAnswer = (q.vocabListeningState.savedOption || q.vocabListeningState.selectedOption || '').trim().toLowerCase().replace(/\s+/g, ' ');
